...
def skin_prices(ids,inp,out):
    print('Getting all case skin prices (Approx 5 mins):')
    skins = []
...
def archive(path,file1,file2,file3):
    if not os.path.exists(path):
        os.makedirs(path)
    files = [file1,file2,file3]
    for file in files:
        print(path+str(add_date(file)))
        shutil.move(str(file), path+str(add_date(file)))
...
def main():
    ...
    #archive(archive_path,case_file,price_file,stash_ids)
